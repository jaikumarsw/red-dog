...
const computeMatchScore = (organization, opportunity) => {
  ...
  // 2. Geography match (20 pts) — location keyword match
  if (!opportunity.keywords || opportunity.keywords.length === 0) {
    ...
    const stateMatch = opportunity.keywords.some((kw) => orgLocation.includes(kw.toLowerCase()));
    if (stateMatch) {
      score += 14;
      reasons.push('Organization location matches opportunity geography (+14 pts)');
    } else if (!orgLocation) {
      score += 10;
  ...
  if (!opportunity.deadline) {
    ...
    if (daysUntil < 0) {
      ...
      score += 3;
      reasons.push(`Deadline very soon (${daysUntil} days) — limited time (+3 pts)`);
    } else if (daysUntil <= 21) {
      score += 7;
      reasons.push(`Deadline within 3 weeks (${daysUntil} days) (+7 pts)`);
    } else {
      score += 10;
  ...
  // 6. Urgency/timeline match (10 pts)
  ...
  return { fitScore, reasons, disqualifiers, recommendedAction };
};

// ─── Service functions ───────────────────────────────────────────────────────

const getAll = async ({ page = 1, limit = 20, organizationId, status, minScore, maxScore }) => {
  const query = {};
...
const computeAndSave = async (opportunityId, organizationId) => {
  ...
  const { fitScore, reasons, disqualifiers, recommendedAction } = computeMatchScore(org, opp);

  const match = await Match.findOneAndUpdate(
    { organization: organizationId, opportunity: opportunityId },
    { fitScore, reasons, disqualifiers, recommendedAction, lastUpdated: new Date(), status: 'pending' },
...
const computeAllForOrganization = async (organizationId) => {
  ...
  for (const opp of opportunities) {
    try {
      ...
      await Match.findOneAndUpdate(
        ...
        { fitScore, reasons, disqualifiers, recommendedAction, lastUpdated: new Date() },
  ...
  await Organization.findByIdAndUpdate(organizationId, { lastMatchRecomputedAt: new Date() });
