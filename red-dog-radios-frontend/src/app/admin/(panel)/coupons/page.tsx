...
export default function AdminCouponsPage() {
  ...
  const handleDeactivate = async (id: string) => {
    ...
    try {
      await adminApi.patch(`coupons/${id}/deactivate`);
      fetchCoupons();
    } catch (err) {
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
          <DialogTrigger asChild>
            <Button className="flex items-center gap-2 bg-[#ef3e34] text-white hover:bg-[#d9382e]">
      ...
      <div className="relative">
        ...
        <Input
          ...
          className="h-11 border-[#e5e7eb] bg-white pl-10"
          value={searchTerm}
            ...
            <tbody className="divide-y divide-[#f3f4f6]">
              {loading ? (
                ...
                filteredCoupons.map((coupon) => (
                  <tr key={coupon._id} className="group transition-colors hover:bg-[#f9fafb]">
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="flex items-center gap-2 font-bold text-[#111827]">
                          <Ticket className="h-3.5 w-3.5 text-[#ef3e34]" />
                          {coupon.code}
                        </span>
                        <span className="line-clamp-1 text-xs text-gray-500">{coupon.description || "No description"}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      {coupon.isActive ? (
